<div class="my-10">
    ...
    <div class="flex flex-wrap gap-x-14 gap-y-5  mt-5 border border-[#343a40] p-5">
        ...
        {{-- @dump($boat->silimarBoats->pluck('similar_boat_id')->contains(6)) --}}
        @foreach ($boats as $similar_boat)
        {{-- @dump($similar_boat->id) --}}
        <div class="flex items-center">     
            <input type="checkbox" id="{{$similar_boat->name}}" name="similar_boats[]" value="{{$similar_boat->id}}"  @if (isset($boat) && $boat->silimarBoats->pluck('similar_boat_id')->contains($similar_boat->id)) checked @endif>
